...
step2_time   = 10.0     # Total ramp time
step2_dt     = 0.003
Nstep2       = step2_time/step2_dt
#

#b_app_mag = Expression(("magnitude*t"),
b_app_mag = Expression(("magnitude - magnitude*2*2*abs( (t-0.5*Tramp)/(2*Tramp) - floor((t-0.5*Tramp)/(2*Tramp) + 0.5) ) "),
                  magnitude = b_app_max, t=0, Tramp = step2_time/2, degree=1)
#
...
plt.plot(time_out[0:ind]-T_tot, -(disp_out[0:ind]-disp_out[np.where(time_out==T_tot)]), linewidth=2.5, \
