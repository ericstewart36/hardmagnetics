...
b_app_max = Constant(50.0)

# No applied b-field for this study
b_app = Constant(as_vector([0.0, 0.0, 0.0]))
...
dt = T_tot/100 # time step size, seconds
...
#
# Tensor functions for internal variable at previous step.
A1_old = Function(W3)
#
...
# Output file setup
file_results = XDMFFile("results/hard_magnetic_mechanical_impulse.xdmf")
...
while (round(t+dt,4) <= round(T_tot,4)):
...
midspan_disp_current = w(scaleX/2, scaleY, scaleZ/2)[1]
...
step2_time   = 01.0     # Total time to observe free oscillation, s
Ndisp        = step2_time/midspan_disp_time 
#
midspan_disp_exp = Expression("midspan_disp_current + max(midspan_disp_max*t/Tramp, midspan_disp_max)",\
                          midspan_disp_current = midspan_disp_current, midspan_disp_max = midspan_disp_max,\
                              t=0, Tramp=midspan_disp_time, degree=1)

# New BC set, including midspan displacement
bcs_e  = DirichletBC(ME.sub(0).sub(1),midspan_disp_exp,facets,8) # right face x move

bcs2 = [bcs_a, bcs_a1, bcs_b, bcs_c, bcs_d, bcs_d1, bcs_e]

# Reset the coupled problem with this set of BCs    
CoupledProblem = NonlinearVariationalProblem(L, w, bcs2, J=a)
...
# smaller time step for dynamic effects
dt = 0.003 # 3 ms
dk.assign(dt)

while (round(t,4) <= round(T_tot + midspan_disp_time,4)):

    # Update time, time-varying BCs
    t += dt
    midspan_disp_exp.t = t - T_tot
...
# Reset the coupled problem to original BCs for free oscillation   
CoupledProblem = NonlinearVariationalProblem(L, w, bcs, J=a)
...
while (round(t,4) <= round(T_tot + Ndisp*midspan_disp_time,4)):
...
ind2 = np.where(time_out==T_tot)[0][0] + 1

'''
# Linear displacement for plotting
buckle_disp = disp_tot*time_out/T_tot

plt.plot(scaleX+buckle_disp[0:ind2], disp_out[0:ind2], linewidth=1.0, \
         marker='o', markersize=3.0, color=colors[3] )
plt.axvline(120.3, c='k', linewidth=1.)
...
plt.plot(time_out[0:ind]-T_tot, disp_out[0:ind]-disp_out[ind], linewidth=2.5, \
...
plt.grid(linestyle="--", linewidth=0.5, color='b')
plt.ylim(-0.4, 0.25)
plt.xlim(0.0,Ndisp*midspan_disp_time)
